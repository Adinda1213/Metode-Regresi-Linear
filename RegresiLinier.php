...
<head>
    ...
    <style>
        ...
        .result table th {
            background-color:rgb(149, 193, 235);
            color: #333;
        }
        .error {
            color: red;
...
<body>
    <div class="container">
        ...
        <?php
        class RegresiLinier {
            public $x, $y, $n, $x2, $y2, $xy, $a, $b, $all, $sst, $ssr, $sse, $r_squared;
            ...
            public function calculate_errors() {
                $mean_y = array_sum($this->y) / $this->n;
            
                // SST: Total Sum of Squares
                $this->sst = array_sum(array_map(fn($yi) => ($yi - $mean_y) ** 2, $this->y));
            
                // SSR: Regression Sum of Squares
                $this->ssr = array_sum(array_map(fn($predicted) => ($predicted - $mean_y) ** 2, $this->all));
            
                // SSE: Error Sum of Squares
                $this->sse = $this->sst - $this->ssr;
            }

            public function calculate_r_squared() {
                if ($this->sst > 0) {
                    $this->r_squared = $this->ssr / $this->sst;
                } else {
                    $this->r_squared = null;
                }
            }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            try {
                ...
                echo "<table>";
                echo "<tr><th>X</th><th>Prediksi Y</th></tr>";
                foreach ($x as $index => $xi) {
                    $prediksi = $regresi->forecast($xi);
                    echo "<tr><td>$xi</td><td>" . number_format($prediksi, 4) . "</td></tr>";
                }
                echo "</table>";
                ...
                echo "<h3>Perhitungan Error</h3>";
                echo "<p><strong>SST (Total Sum of Squares):</strong> " . number_format($regresi->sst, 4) . "</p>";
                echo "<p><strong>SSR (Regression Sum of Squares):</strong> " . number_format($regresi->ssr, 4) . "</p>";
                echo "<p><strong>SSE (Error Sum of Squares):</strong> " . number_format($regresi->sse, 4) . "</p>";
                if (!is_null($regresi->r_squared)) {
                    echo "<p><strong>R-squared:</strong> " . number_format($regresi->r_squared, 4) . "</p>";
                } else {
                    echo "<p><strong>R-squared:</strong> Tidak dapat dihitung (SST = 0)</p>";
                }
